...
- [ ] Segmented multi-connection HTTP/HTTPS downloading with resume
- [ ] FTP support
- [ ] Checksum verification (MD5/SHA-1/SHA-256), like aria2's `--checksum`
...
- [ ] **Pixeldrain** (easy — confirmed via its actual docs) — has a real public REST API, no scraping needed: `GET https://pixeldrain.com/api/file/{id}` returns the raw file directly, supports HTTP byte-range requests (pairs perfectly with the segmented HTTP engine from earlier in this phase), and needs no auth for anonymous downloads. Free tier has per-IP rate/transfer limits and may require a CAPTCHA past a threshold — surface that error clearly rather than silently failing.
- [ ] **Catbox.moe** (trivial) — uploaded files already get a permanent, static, direct URL (`files.catbox.moe/xxxx.ext`); there's nothing to resolve at all, it's just an HTTP download like any other. Worth including mainly because it's zero extra engineering. Note: prohibits `.exe`/`.doc*`/`.jar` and a few other extensions, and disallows commercial hotlinking per their FAQ.
- [ ] **Gofile.io** (candidate, needs confirmation) — reportedly has a documented public API for direct links, but this wasn't independently verifiable via a plain fetch when researched (likely a JS-rendered docs site) — confirm the actual API surface before committing to it as a resolver target.
- [ ] **Workupload** — investigated and **deprioritized for now**: every page fetched (including their FAQ) returned a Cloudflare-style "checking that you are not a robot" challenge instead of content, and no public API was discoverable. Building a resolver would mean solving a bot-check/JS challenge rather than calling a documented endpoint — fragile, and edges toward circumventing their anti-automation protection rather than just fetching a file. Revisit only if they publish a real API later.
- [ ] Design the Resolver interface so it's trivial to add more hosts later without more core changes — ship with the confirmed-easy ones (Dropbox, Pixeldrain, Catbox) first, then Google Drive/Mediafire/Gofile, then Mega.nz.
