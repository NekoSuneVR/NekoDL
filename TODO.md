...
- **Real bug found by live use**: downloading a Google Drive share link produced a file named like `19ff622e86ddc9db-uc` instead of its real name (e.g. "Terms of Use License for Mayo_EN.pdf"). Root cause: the destination filename was guessed once, up front, from the *resolved* URL's path (`taskFilename` in `addtask.go`) — but Google Drive's resolved URL is `drive.google.com/uc?export=download&id=...`, whose path is just `/uc`, so that's exactly the "filename" it produced. The real name only ever existed in the final response's `Content-Disposition` header, which `httpengine` never read. Fixed generally (not just for Google Drive) — every one-click-hoster resolver benefits: `httpengine`'s `probe()` now also parses `Content-Disposition` (`filename=` and the RFC 5987 `filename*=` percent-encoded form), and if present, the task renames its destination to `<same-id-prefix>-<real filename>` before writing any data. Server-supplied filenames are sanitized (`path.Base` after normalizing both slash styles) against a malicious/misbehaving server trying to name a file `../../etc/passwd`. Verified with 2 new tests reproducing the exact bug shape.
...
- [x] Maintain NekoDL's patch set for upstream bugs/site breakage not yet merged — **mechanism implemented, no patches needed yet**. The `ytdlp-builder` Docker stage clones yt-dlp's own git repo at the `YTDLP_VERSION` tag, applies every `*.patch` file found in `docker/patches/yt-dlp/` (`git apply`, filename order), then builds and installs a wheel from that (possibly-patched) source instead of installing the prebuilt PyPI package. No patches exist yet, so today this is equivalent to the stock release — but adding one is a drop-a-file-and-rebuild operation, not a Dockerfile rewrite. See `docker/patches/yt-dlp/README.md` for the fork-vs-local-patch reasoning and naming/ordering convention. **Verified in real CI**: the initial `python3 -m ensurepip`-based approach genuinely failed on Alpine (caught by a real failed GitHub Actions run, not guessed at), and the `ytdlp-builder` rewrite was watched to a real success with both `linux/amd64`/`linux/arm64` confirmed present in the pushed manifest.
...
- [x] Global + per-task settings — per-task settings (priority, HTTP max-connections, torrent proxy/seed/bandwidth limits) implemented in the Add Download modal's "Options" tab. **Global settings** now real too: a new `core/internal/settings` package (`GET`/`PUT /api/v1/settings`, persisted to `settings.json`, 4 unit tests) with `allow_torrents` and `require_proxy_for_torrents` — the latter is genuinely enforced server-side in `handleAddTorrent` (rejects the request outright), not just a UI preference. Both default to permissive (torrents allowed, proxy not required) so the existing "warn but don't block" behavior is unchanged unless a user explicitly opts into stricter enforcement. Dashboard: a Settings modal (toggle switches, `web/src/components/ui/Toggle.tsx`) below API Token in the sidebar, auto-saving on change. **Live-verified all four states end-to-end**: default (allowed+warned), proxy required (rejected 400), torrents disabled (rejected 403), and reverting back to default.
...
- [x] **Real-world hardware confirmation, and a real bug it caught**: pulled and ran on an actual Docker host, which immediately surfaced `mkdir /data/downloads: permission denied` — this environment has no Docker, so this was never going to be caught here. Root cause: the image's `chown -R nekodl:nekodl /data` only takes effect for the image's own build-time layer; a *mounted* volume's ownership comes from wherever Docker (or the host, for a bind mount) actually created it, which can easily end up root-owned regardless of what the image says, and an existing volume from an older image version (from before `USER nekodl` existed, or one that ran as root) keeps its old ownership across an upgrade rather than getting fixed automatically. Fixed with the standard pattern for this (same approach linuxserver.io-style images use): `docker/entrypoint.sh` stays root long enough to `chown -R nekodl:nekodl /data` on every container start, then drops to the nekodl user via `su-exec` before running anything else — idempotent, so it's a no-op on a volume that's already correctly owned.
