...
- [ ] Wire `megalink.Downloader` into `POST /api/v1/tasks` (currently only resolver-based and plain-URL downloads are reachable from the API).
- [ ] Verify the Google Drive large-file confirm-token flow and the MediaFire/Mega.nz live API assumptions against real accounts/files when one becomes available.
