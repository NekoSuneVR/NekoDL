...
### Notes
- FTP support, Google Drive/Mediafire/Gofile resolvers, and Mega.nz are still not implemented — see [TODO.md](TODO.md) Phase 2 for exact status and the reasoning behind deferring each one (mainly: hand-rolling is the wrong call for FTP and especially Mega.nz's encryption; an existing library is safer than reuse-avoidance for its own sake).
