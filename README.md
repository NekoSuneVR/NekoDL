...
> ⚠️ **Status: pre-alpha.** The core service (`core/`) has a working scheduler, REST API, real WebSocket progress feed, an HTTP engine, a BitTorrent engine, a yt-dlp engine, a BoothDownloader engine, and most one-click-hoster resolvers — all build- and test-verified. The Web GUI (`web/`) has a real, working task list, add-download flow (URL/magnet/`.torrent` with auto-detect, plus dedicated yt-dlp and Booth options), pause/resume/cancel/remove, a settings screen, and live WebSocket updates — also live-verified against the real server, not just built. Still missing: the Plex-ripper engine; FTP; a handful of one-click hosters; YouTube subscriptions; per-task file/peer/log detail (no backend endpoint for it yet). See [TODO.md](TODO.md) for exactly what's done vs. planned per phase, and [CHANGELOG.md](CHANGELOG.md) for the detailed history.
...
- 📁 **One-click hoster support** — Dropbox, Pixeldrain, Google Drive, and Mediafire resolvers are built and tested; Catbox.moe needs no resolver at all (it's already a direct URL); Mega.nz has its own full download+decrypt pipeline. Gofile was investigated and turns out to require a paid account for API-based link resolution, so it's not supported. See TODO.md for exactly what's live-verified vs. not per site.
...
Difficulty varies sharply by site, and so does how confident you should be in each — some of the below are live-verified, others are best-effort:

- **Dropbox** — easy; shared links already support a direct-download URL form, just a rewrite. Built and tested.
- **Pixeldrain** — easy; has a real, documented public API (`GET /api/file/{id}`, no auth, byte-range support) — no scraping at all. Built, tested, and its live API endpoint independently confirmed reachable.
- **Catbox.moe** — trivial; uploaded files already get a permanent static URL, there's nothing to resolve. Confirmed live with a real anonymous upload and a real range request.
- **Google Drive** — moderate; the ID-extraction and direct-download-URL rewrite is built, tested, and its redirect behavior confirmed against Google's real server. The large-file "can't scan for viruses, download anyway?" confirmation flow is **not implemented** — no Google account was available to verify it, so it wasn't guessed at.
- **Mediafire** — moderate but fragile; no public API for arbitrary files, so the resolver scrapes the share page for the real CDN link and will need periodic fixes when the site's markup changes. Built and tested against constructed mock HTML — **not verified against a real share page** (every real Mediafire link found during research was malware-flagged, so none were used as test fixtures).
- **Gofile.io** — investigated and **not supported**: its API is real, but resolving a download link via `GET /contents/{id}` requires a **paid account** — confirmed twice, including with the uploading account's own guest token on its own file. Not a "maybe later," a hard requirement that doesn't fit a free resolver.
- **Mega.nz** — a different category of problem, not just scraping: Mega encrypts everything client-side with a key embedded in the URL fragment, so decryption has to happen while streaming the download rather than via a URL rewrite. No actively-maintained Go library actually supports downloading from someone else's public link (the one that's maintained requires an authenticated account; the ones that support public links haven't been updated since 2019–2021), so NekoDL implements the documented key-derivation/decryption scheme directly — using only stdlib `crypto/aes`/`crypto/cipher` for the actual cipher, transcribed precisely from a long-standing reference implementation rather than reverse-engineered from scratch. Verified with a full pipeline test against independently-encrypted mock data; **not verified against the real, live MEGA API**.
